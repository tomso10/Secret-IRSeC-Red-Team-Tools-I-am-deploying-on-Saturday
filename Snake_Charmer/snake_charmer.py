...
def single_connection_command_windows(values_tuple, commmand=None, hostname_in=None):
    ...
    try:
        ...
        session = winrm.Session(endpoint, auth=(username_in, password_in), transport='ntlm')

        # Run command in a thread to enforce timeout
        def _run():
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                print(Style.BRIGHT, Fore.RED, "Unexpected error on host ", Fore.BLUE, hostname,
                      Style.RESET_ALL, ", ", e, sep="")
                return

        # Decode stdout and stderr
        stdout = (result.std_out or b"").decode(errors="ignore")
        stderr = (result.std_err or b"").decode(errors="ignore")

        # Check command exit status
        if result.status_code == 0:
            print(Fore.BLUE, hostname, ": ", Fore.GREEN, stdout, Style.RESET_ALL, sep="")
        else:
            err_msg = stderr if stderr else "<no error provided>"
            print(Style.BRIGHT, Fore.RED, "Error executing command on box: ", Fore.BLUE, hostname,
                  Style.RESET_ALL, ", ", err_msg, sep="")

    except winrm.exceptions.UnauthorizedError:
        print(Style.BRIGHT, Fore.RED, "Error! Invalid credentials for box: ", Fore.BLUE, hostname,
              ", ", Fore.YELLOW, username_in, ":", password_in, sep="")
    except Exception as e:
        print(Style.BRIGHT, Fore.RED, "Unexpected error on host ", Fore.BLUE, hostname,
              Style.RESET_ALL, ", ", e, sep="")
...
def single_connection_command_linux(values_tuple, commmand=None, hostname_in=None):
    ...
    try:
        ...
        print(Style.BRIGHT,Fore.RED,"Error! Invalid credentials for box: ",Fore.BLUE,hostname,", ",Fore.YELLOW,username_in,":",password_in,sep="")
    except paramiko.SSHException as e:
        print("ssh error,",e)
    except TimeoutError:
        print(Style.BRIGHT,Fore.RED,"Box timed out! host ",Fore.BLUE,hostname,Fore.RED," is not responding, likely down or fire walled off",Style.RESET_ALL,sep="")
    except Exception as e:
        print(Style.BRIGHT,Fore.RED,"Unexpected error on host ",Fore.BLUE,hostname,Style.RESET_ALL,", ",e,sep="")
